...
class Chart(object):
    ...
    def _populate_bpm_event_timestamps(self):
        ...
        for i, cur_event in enumerate(iterate_from_second_elem(self.sync_track.bpm_events), start=1):
    ...
    def _populate_event_timestamps(self, events):
        ...
        def idx_of_proximal_bpm_event(event, start_idx=0):
            for idx in range(start_idx, len(self.sync_track.bpm_events)):
                ...
                next_bpm_event = self.sync_track.bpm_events[idx+1] if not is_last_bpm_event else None
                if is_last_bpm_event or next_bpm_event.tick > event.tick:
                    return idx

        proximal_bpm_event_idx = 0
        for i, event in enumerate(events):
            proximal_bpm_event_idx = idx_of_proximal_bpm_event(event, start_idx=proximal_bpm_event_idx)
            ...
            timedelta_since_proximal_bpm_event = datetime.timedelta(seconds=seconds_since_proximal_bpm_event)
